...
import { yearsExp } from "../common/commonVariables";
...
export const HeroSection = () => {
  ...
  const codeSnippets = [
    "import { FullStackDeveloperDataEngineer } from 'harvey.marshall';",
    "",
    "const developer = new FullStackDeveloperDataEngineer({",
    "  name: 'Harvey',",
    "  stack: ['React', 'Python', 'SQL'],",
    "  focus: 'Building scalable solutions and software',",
    "  status: 'Open to new opportunities and roles'",
    "});",
    "",
    "await developer.launchPortfolio();",
    "// Featured: SaaS, InternalTooling, WebDevelopment, DataIngestion",
    "",
    "developer.connect();",
  ...
  const achievements = [
    { number: `${yearsExp}+`, label: "Years in Production", icon: <Shield className="h-3 w-3" /> },
    { number: `${reposNumber}+`, label: "Projects Delivered", icon: <TrendingUp className="h-3 w-3" /> }
  ];
        ...
        <motion.div className="flex flex-col lg:flex-row items-center justify-between gap-16 lg:gap-20" initial="hidden" animate={isInView ? "visible" : "hidden"} variants={{ hidden: { opacity: 0 }, visible: { opacity: 1, transition: { staggerChildren: 0.25, delayChildren: 0.5 } } }}>
          ...
          <div className="flex-1 text-center lg:text-left max-w-2xl mx-auto lg:mx-0">
            ...
            <motion.h1 className="text-4xl sm:text-5xl md:text-6xl lg:text-7xl font-bold leading-tight tracking-tight" variants={{ hidden: { y: 30, opacity: 0 }, visible: { y: 0, opacity: 1, transition: { duration: 0.8 } } }}>
              <span className="block text-foreground">I'm Harvey</span>
            ...
            <motion.p className="text-lg sm:text-xl text-muted-foreground mt-6 leading-relaxed max-w-2xl" variants={{ hidden: { y: 30, opacity: 0 }, visible: { y: 0, opacity: 1, transition: { duration: 0.8 } } }}>
              I build <span className="text-primary font-semibold">high-performance software and tooling</span> that drive business growth. Specializing in Python, React and scalable architecture for startups and enterprises.
            </motion.p>

            <motion.div className="grid grid-cols-2 sm:grid-cols-2 gap-4 my-8" variants={{ hidden: { y: 30, opacity: 0 }, visible: { y: 0, opacity: 1, transition: { duration: 0.8 } } }}>
            ...
            <motion.div className="flex flex-col sm:flex-row gap-4 justify-center" variants={{ hidden: { y: 30, opacity: 0 }, visible: { y: 0, opacity: 1, transition: { duration: 0.8 } } }}>
